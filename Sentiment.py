...
# (3) precision and recall for the negative class; (4) F1 score;
    # --------- Evaluation metrics for Step 2 ---------
    # Accuracy
    accuracy = correct / float(total) if total > 0 else 0.0

    # Positive class: precision and recall
    precision_pos = correctpos / float(totalpospred) if totalpospred > 0 else 0.0
    recall_pos    = correctpos / float(totalpos)     if totalpos > 0     else 0.0

    # Negative class: precision and recall
    precision_neg = correctneg / float(totalnegpred) if totalnegpred > 0 else 0.0
    recall_neg    = correctneg / float(totalneg)     if totalneg > 0     else 0.0
    ...
    # Print results
    print("Results for:", dataName)
    print("  Total samples:   ", total)
    print("  Accuracy:        {:.4f}".format(accuracy))
    print("  Positive class - Precision: {:.4f}, Recall: {:.4f}".format(precision_pos, recall_pos))
    print("  Negative class - Precision: {:.4f}, Recall: {:.4f}".format(precision_neg, recall_neg))
    print("  F1 (positive):   {:.4f}".format(f1_pos))
    print()
...
def testDictionaryImproved(sentencesTest, dataName, sentimentDictionary, threshold):
    ...
    # 定义否定词、加强词和减弱词（全部小写，跟 token 对齐）
    negations = {"not", "no", "never", "n't", "hardly", "rarely"}
    intensifiers = {"very", "really", "extremely", "so", "incredibly",
                    "too", "highly", "quite", "terribly", "remarkably"}
    diminishers = {"slightly", "somewhat", "barely", "little", "bit"}
    ...
    print("Results for (improved):", dataName)
    print("  Total samples:   ", total)
    print("  Accuracy:        {:.4f}".format(accuracy))
    print("  Positive class - Precision: {:.4f}, Recall: {:.4f}".format(precision_pos, recall_pos))
    print("  Negative class - Precision: {:.4f}, Recall: {:.4f}".format(precision_neg, recall_neg))
...
# run improved rule-based classifier on datasets
#testDictionaryImproved(sentencesTrain,  "Films (Train Data, Improved Rules)\t", sentimentDictionary, 1)
#testDictionaryImproved(sentencesTest,   "Films  (Test Data, Improved Rules)\t", sentimentDictionary, 1)
#testDictionaryImproved(sentencesNokia,  "Nokia   (All Data, Improved Rules)\t", sentimentDictionary, 1)
